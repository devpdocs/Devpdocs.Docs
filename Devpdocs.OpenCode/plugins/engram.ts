...
const ENGRAM_REQUEST_TIMEOUT_MS = 5_000
...
async function engramFetch(
  ...
  const timeout = setTimeout(() => controller.abort(), ENGRAM_REQUEST_TIMEOUT_MS)
...
export const Engram: Plugin = async (ctx) => {
  ...
  async function ensureSession(sessionId: string): Promise<void> {
    ...
    const registered = await engramFetch("/sessions", {
      method: "POST",
      body: {
        id: sessionId,
        project,
    ...
    if (registered !== null) knownSessions.add(sessionId)
  }
